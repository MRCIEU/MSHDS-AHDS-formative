...
1. Login to bluepebble using VSCode
...
3. Navigate to `$WORK/projects` and clone your GitHub repository there.
    - Get the git repository's SSH URL from your project's GitHub page
...
You have previously setup a `slurm_profile` that can specify how the Snakemake pipeline can be executed on the HPC worker nodes. See the `config.yaml` file here: https://github.com/CharlotteJames/ahds-week9. You can 

1. Copy that `config.yaml` file to your formative project directory on bluepebble
2. Try running your snakemake pipeline with `snakemake --profile .`

Note that it may be prudent to run this within a `screen` session (why?)
